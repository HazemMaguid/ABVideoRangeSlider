...
public protocol ABVideoRangeSliderDelegate {
    ...
    func didFinishChangeValue(startTime: Float64, endTime: Float64)
    func indicatorDidChangePosition(videoRangeSlider: ABVideoRangeSlider, position: Float64)
    func didFinishChangePosition(position: Float64)
}
...
public class ABVideoRangeSlider: UIView {
    ...
    let indicatorWidth: CGFloat = 16.0
...
//        self.addSubview(endTimeView)
    ...
    @objc func startDragged(recognizer: UIPanGestureRecognizer){
        ...
        if recognizer.state == .ended {
            self.delegate?.didFinishChangeValue(startTime: startSeconds, endTime: endSeconds)
        }
    ...
    @objc func progressDragged(recognizer: UIPanGestureRecognizer){
        ...
        if recognizer.state == .ended {
            self.delegate?.didFinishChangePosition(position: progressSeconds)
        }
    ...
    @objc func viewDragged(recognizer: UIPanGestureRecognizer){
        ...
        let spaceBetweenEndStart = endPosition - startPosition
        ...
        if startPosition < 0 {
            startPosition = 0
            endPosition = min(abs(spaceBetweenEndStart), self.frame.size.width)
            progressPosition = progressPosition - translation.x
        }
        
        if endPosition > self.frame.size.width{
            endPosition = self.frame.size.width
            startPosition = max(endPosition - spaceBetweenEndStart, 0)
            progressPosition = progressPosition - translation.x
        }
